		...
		if (meta.getTotal_count() > 15) { // 검색된 장소가 15개를 넘어갈 경우
			for (int i = 1; i <= Math.min(meta.getTotal_count() / 15, 15); i++) { // 15개씩 나눠서 검색
				...
				for (Places.Document document : documents2) {// enhanced for문으로 documents2안의 document들을 각각 탐색하여 Pharmacy
					...
					if (firstPageMarker > 0) {
						Pharmacy pharmacy2 = new Pharmacy(Double.parseDouble(document.getX()), // document의 x, y값을
																								// Pharmacy 객체에 담아
																								// pharmacyList에 저장
								Double.parseDouble(document.getY()), document.getPlace_name(), document.getPhone(),
								document.getPlace_url());// document의 place_name, phone값을 Pharmacy 객체에 담아 pharmacyList에
															// 저장
